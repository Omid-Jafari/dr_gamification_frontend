...
const Champ = ({
  ...
  handleNextModal,
  closeResultFunc,
  finishedQuestionsQuery,
}: {
  open: boolean;
  user: User;
  handleNextModal: (data: any, isLast?: boolean) => void;
  closeResultFunc: () => void;
  finishedQuestionsQuery: UseQueryResult<AxiosResponse<any, any>, unknown>;
}) => {
  ...
  return (
    <>
      {finishedQuestionsQuery?.isLoading ? (
        <div className="h-full w-full flex items-center justify-center">
          <Loading className="text-[#b00803] !w-11 !h-11" />
        </div>
      ) : (
        <div
              ...
              {user.secondTimeResult
                ? `${
                    Math.floor(
                      ...
                      ? `${Math.floor(
                          (+user?.secondTimeResult?.time %
                            (1000 * 60 * 60 * 24)) /
                            (1000 * 60 * 60)
                        )}:`
                      : ""
                  }${Math.floor(
                    (+user?.secondTimeResult?.time % (1000 * 60 * 60)) /
                      (1000 * 60)
                  )}':${Math.floor(
                ...
                ? `${
                    Math.floor(
                      ...
                      ? `${Math.floor(
                          (+user?.firstTimeResult?.time %
                            (1000 * 60 * 60 * 24)) /
                            (1000 * 60 * 60)
                        )}:`
                      : ""
                  }${Math.floor(
                    (+user?.firstTimeResult?.time % (1000 * 60 * 60)) /
                      (1000 * 60)
                  )}':${Math.floor(
